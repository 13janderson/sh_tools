...
if [[ "$NVIM" ]]; then
  selected=$(tac $HISTFILE | sed -E "/$me/d" | sed -E 's/(: [0-9]*:0;)(.*)/\2/' | awk '!seen[$0]++' | fzf --height=50% -m --bind "ctrl-y:execute-silent(echo {} | tr -d '\n' | eval $COPY_CMD; echo y > $TMP_FLAG)+accept")
else
  selected=$(tac $HISTFILE | sed -E "/$me/d" | sed -E 's/(: [0-9]*:0;)(.*)/\2/' | awk '!seen[$0]++' | fzf --tmux 70% -m --bind "ctrl-y:execute-silent(echo {} | tr -d '\n' | eval $COPY_CMD; echo y > $TMP_FLAG)+accept")
fi
